...
class DaleChallReadabilityFormula(BaseReadability):
    ...
    slug = 'dale_chall_readability_formula'
    ...
    def calc(self, text: str) -> ReadingLevel:
        ...
        not_difficult_words = len(text_not_difficult_words)

        result = 0.1579 * (difficult_words / words * 100.0) + 0.0496 * (words / sentences)

        if difficult_words / not_difficult_words > 0.05:
            result = result + 3.6365
        ...
        if result < 3.5:  # this is made up
            ...
            level = 16
        elif result < 12:  # this is made up
            level = 17
